...
import roleImage from "../assets/images/role_selection.png";
...
const sections = [
  {
    ...
    description: (
      <p>
        ...
        select your role. As a Repurposer, click on the "Repurposers" button.
...
const Repurposers: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const elements = resourcesRef.current?.querySelectorAll(
      ".resources-content, .resources-banner-text, .resources-info-section, .resources-info-section2"
    );
  ...
  return (
    <div className="resources" ref={resourcesRef}>
      <div className="resources-banner">
        <h1 className="roboto-slab">REPURPOSERS</h1>
      </div>
      <section className="resources-container">
        {sections.map((section, index) => (
          <div
            key={index}
            className={`resources-content ${index % 2 !== 0 ? "reverse" : ""}`}
          >
